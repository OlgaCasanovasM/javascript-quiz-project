document.addEventListener("DOMContentLoaded", () => {
  ...
  const quiz = new Quiz(questions, quizDuration, quizDuration); // why do we have 2 arguments looking the same ????
  ...
  function showQuestion() {
    ...
    question.choices.forEach(function (choice) {
      const choiceRow = document.createElement("li");
      choiceRow.innerHTML = `<input type="radio" name="choice" value="${choice}">
      <label>${choice}</label>
    <br>`;
  ...
  function nextButtonHandler() {
    // we should get the user's answer after a click = event to track.

    let selectedAnswer = ""; // A variable to store the selected answer value

    const allChoices = choiceContainer.querySelectorAll("input"); // "input[name="choice"]"
